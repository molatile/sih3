...
filepath = 'D:/ai man/sih3/jago-portal/src/pages/RegistrationPage/RegistrationPage.jsx'
        ...
        setAppState(p => ({
            ...p, registrationDraft: {
                ...
                documents: {
                    ...
                    [docKey]: { status: 'uploaded', fileName: docKey + '_doc.pdf' }
                }
            }
        }));
    };
                    ...
                    <div className="flex items-center justify-between mb-8 relative">
                        ...
                        {[1, 2, 3, 4].map(step => (
                            <div key={step} className="flex flex-col items-center bg-gray-50 px-2">
                                ...
                                <span className={	ext-xs mt-1 font-medium }>
                    ...
                    {currentStep === 3 && (
                        <div>
                            <div className="flex justify-between items-center mb-4 border-b pb-2">
                                ...
                                <span className="text-xs bg-blue-50 text-blue-600 px-2 py-1 rounded">AI-powered OCR coming soon</span>
                            </div>
                            <div className="space-y-4">
                                {docsList.map(doc => (
                                    <div key={doc.key} className="flex items-center justify-between p-3 border rounded">
                                        ...
                                        {draft.documents[doc.key]?.status === 'uploaded' ? (
                                            <span className="text-green-600 text-sm font-bold">Uploaded ✔</span>
                                        ) : (
                                            <button onClick={() => handleUpload(doc.key)} className="px-3 py-1 bg-gray-100 border text-sm rounded hover:bg-gray-200">Upload</button>
                                        )}
                                    </div>
                                ))}
                            </div>
